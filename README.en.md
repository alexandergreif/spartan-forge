...
9. [The FDE Workflow in Detail](#9-the-fde-workflow-in-detail)
10. [CLI Reference (sync.ts)](#10-cli-reference-syncts)
11. [Team Workflow: managing multiple repos](#11-team-workflow-managing-multiple-repos)
12. [Creating custom agents (via Leonidas)](#12-creating-custom-agents-via-leonidas)
...
Each agent writes a machine-readable **STATUS line** to `tasks/notes.md` when done. Leonidas reads this before spawning the next agent — if an agent reports `BLOCKED` or `BUILD_FAILED`, the pipeline halts and surfaces the problem to the user. A `REQUEST_CHANGES` STATUS from the reviewer always pauses for human decision; Leonidas never auto-fixes.
...
7. fde-reviewer:
   ...
   → STATUS: APPROVE (or REQUEST_CHANGES with specific items)
...
## 11. Team Workflow: managing multiple repos
...
## 12. Creating custom agents (via Leonidas)
