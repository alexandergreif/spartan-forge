...
function syncRepo(group: string, targetPath: string) {
  ...
  // First-time only: wiki directories
  const wikiDirs = [
    "raw/research",
    "raw/drafts",
    "wiki/concepts",
  ...
  for (const dir of wikiDirs) {
    ...
    if (!existsSync(dest)) {
      ...
      const gitkeep = resolve(dest, ".gitkeep");
      if (!existsSync(gitkeep)) writeFileSync(gitkeep, "");
      console.log(`  ✅ ${dir}/ created`);
    } else {
      console.log(`  ⏭️  ${dir}/ exists, skipping`);
    }
  }

  // First-time only: wiki/index.md from template
  const wikiIndex = resolve(target, "wiki/index.md");
  if (!existsSync(wikiIndex)) {
    const tmpl = resolve(TEMPLATES_DIR, "wiki", "index.md.template");
    if (existsSync(tmpl)) {
      copyFileSync(tmpl, wikiIndex);
      console.log("  ✅ wiki/index.md created from template");
    }
  } else {
    console.log("  ⏭️  wiki/index.md exists, skipping");
  }
}
...
function uninstallRepo(targetPath: string) {
  ...
  if (existsSync(agentsTarget)) {
    ...
    for (const sub of readdirSync(agentsTarget, { withFileTypes: true })) {
      if (sub.isDirectory()) {
        ...
        if (readdirSync(subPath).length === 0) rmSync(subPath, { recursive: true });
      }
    }
  ...
  console.log("     tasks/todo.md, tasks/notes.md, tasks/lessons.md, CLAUDE.md, raw/, wiki/");
}
...
function loadRepos(): Array<{ repo: string; path: string }> {
  ...
  return readFileSync(REPOS_CONF, "utf-8")
    ...
    .map(line => {
      ...
      const repo = trimmed.slice(0, spaceIdx);
      const path = trimmed.slice(spaceIdx).trim();
      return { repo, path };
    });
}
...
switch (command) {
  ...
  default:
    console.error("Usage:");
    console.error("  sync.ts install                            # Everything: global agents + commands + leonidas + auto-sync current repo");
    console.error("  sync.ts uninstall                          # Remove spartan-forge agents/commands from ~/.claude/");
    console.error("  sync.ts uninstall <path>                   # Remove spartan-forge files from a project (leaves tasks/, CLAUDE.md, raw/, wiki/)");
    console.error("  sync.ts sync <agent-group> <path>          # Sync agents into a specific repo");
